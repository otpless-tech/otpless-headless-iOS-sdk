...
There is **no sample app in this repo** (unlike the Android spokes) — nothing to gitignore or exclude as a testbed here; all code under `Sources/` is the shipped SDK. There are also, as of this PR, **no tests**, and **no `docs/` directory — by design** (see "Documentation lives in Atlas" below). This PR is the scaffolding half of bringing this repo to agentic readiness: constitution, verification gate, skills, CI.
...
### Documentation lives in Atlas — this repo has no `docs/`

**[`otpless-tech/atlas`](https://github.com/otpless-tech/atlas) is the only home for platform documentation.** It is the internal Mintlify knowledge base for the whole OTPLESS platform; this repo's section is **`repos/otpless-headless-iOS-sdk/`** there. Source repos across the platform no longer carry a `docs/` directory — one page per fact, in one place, instead of N drifting per-repo copies. Do **not** add `docs/SDK-GUIDE.md` (or any other doc page) back into this repo; a doc change belongs in an Atlas PR, and a change here that makes an Atlas page wrong is caught by CI (below).

The guide-first protocol, unchanged in spirit — only its address moved:

1. **Locate via this repo's Atlas section first** — `repos/otpless-headless-iOS-sdk/` in Atlas holds the canonical description of this SDK's code (architecture, flows, response/error contract, networking, telemetry, quirks). Its layout tree and source-file → section map name the exact file/function for any requested behavior. Do not re-derive the architecture from scratch. Read it via the Atlas repo (`gh api`, a local clone, or the rendered site).
2. **Read only the target file(s)** the Atlas page points at, plus immediate collaborators.
3. **Verify before editing** — Atlas describes intent; the code in this repo is the source of truth. When they disagree, the code wins and the Atlas page is a bug to fix in Atlas.
4. **Never edit an Atlas page whose frontmatter says `generated: true`** — fix its source file here; the page regenerates on the next sync (Atlas CI rejects direct edits anyway).

**How the two repos are wired** — entirely by `.github/workflows/atlas-docs.yml`, which calls two reusable workflows Atlas owns:

| Job | Trigger | Reusable workflow | Effect |
|---|---|---|---|
| `verify` | `pull_request` → main | `otpless-tech/atlas/.github/workflows/verify-docs.yml@main` (inputs `source_repo`, `source_sha`; secret `ATLAS_PAT`) | **Fails this PR** if an Atlas page this repo owns is stale w.r.t. the PR head. |
| `sync` | `push` → main | `otpless-tech/atlas/.github/workflows/sync-from-source.yml@main` (inputs `source_repo`, `source_ref`; secret `ATLAS_PAT`) | Regenerates this repo's Atlas pages from the merged source. |

**Both jobs require the `ATLAS_PAT` repository secret** — Atlas is a *private* repo, and in a reusable workflow the caller's `github.token` is scoped to the caller, so it cannot check Atlas out. That includes the read-only `verify` job. Each reusable workflow preflights the secret and fails with an actionable message when it is absent, so a missing `ATLAS_PAT` looks like a configuration error rather than a mysterious 404. One secret configures both; it needs `contents:read` on Atlas for `verify` and `contents:write` for `sync`.

Consequences to keep in mind: `make gate` **cannot** check documentation — it has no Atlas checkout, so `scripts/docs-verify.sh` is deliberately source-side-only (CHANGELOG/podspec/tag/`Constants.SDK_VERSION`/gate-line consistency). Documentation staleness is Atlas's `verify` status on the PR, not a gate rung. Also: Atlas's `.atlas/manifest.yml` has **no entry for this repo yet**, so `verify`/`sync` are no-ops on this repo's pages until that row lands — adding it is a change to the Atlas repo.

`Sources/OtplessBM/Otpless.swift` (the `Otpless` singleton — the entire public entry point) is the best starting map in code, and remains the right first read when an Atlas page is missing or you suspect it is stale.
...
- **`.gitignore` blanket-ignored `docs/` and `CLAUDE.md`** under a "Claude Code / agent session artifacts" comment — the `CLAUDE.md` line would have fought this very PR (an ignored constitution is a constitution nobody reads), and neither line was hiding a real local-only artifact. This PR removes both. Note the `docs/` line becoming moot for a second, independent reason: documentation now lives in Atlas, so this repo has no `docs/` directory to ignore *or* to track.
...
| **docs-sync** | Any code change under `Sources/OtplessBM/`: keeping `CHANGELOG.md` current, and deciding whether an Atlas page under `repos/otpless-headless-iOS-sdk/` needs a companion Atlas PR. |
...
- **Wrapper check:** if the response contract or public API moved, confirm `react-native-headless-lite` and `react-native-headless-sdk` (both pin this iOS SDK per the hub topology) either tolerate it or have a coordinated update filed.
...
- **Git workflow:** work on a feature branch, open a PR, fill the PR template's constitution checklist (including the parity-statement line — the hub's Android ↔ iOS parity rule: "Android typically leads"; a phone-auth-related change here that has an Android counterpart change needs a parity statement). CI on every PR: `build-test` (the gate above + actionlint) and `atlas-docs` (`verify` — is this repo's Atlas documentation still true after your change?). Never push directly to `main`.
- **CI workflows here are `build-test` (the gate above + actionlint) and `atlas-docs` (documentation freshness/sync, delegated to Atlas — see above).** A device-smoke workflow is deliberately NOT added: it needs a provisioned device/simulator matrix this PR didn't set up. Tracked in "Follow-ups" below. There is no in-repo `docs-sync`/`docs-audit` workflow and there should not be one — Atlas owns doc verification for every source repo, so duplicating it here would just be a second, drifting implementation.
...
- **Add a `.atlas/manifest.yml` entry for `otpless-headless-iOS-sdk` in `otpless-tech/atlas`** — without it, `atlas-docs.yml`'s `verify`/`sync` jobs have no rules to act on and this repo's documentation is neither generated nor gated. This is a PR against the Atlas repo, not this one.
- **Configure the `ATLAS_PAT` repository secret here** — both `atlas-docs.yml` jobs need it (Atlas is private; see above). Until it exists, both jobs fail their preflight with an explicit message; `verify` runs on every PR, so this must land before `atlas-docs` is made a required status check.
- Add a device-smoke CI workflow once a device/simulator matrix is provisioned.
