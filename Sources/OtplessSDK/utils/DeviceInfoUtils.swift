...
class DeviceInfoUtils : @unchecked Sendable {
    ...
    func getDeviceInfoString(completion: @escaping (String) -> Void) {
        if let cachedString = deviceInfoString {
        ...
        var deviceInfo: [String: Any] = [
            ...
            "device": device.name,
            "model": UIDevice.modelName,
            "iOS_version": "\(os.majorVersion).\(os.minorVersion)",
            "product": device.systemName,
            "hardware": hardwareString(),
        ]
        
        let screenSize = UIScreen.main.bounds
        deviceInfo["screenWidth"] = Int(screenSize.width)
        deviceInfo["screenHeight"] = Int(screenSize.height)
        var userAgent = WKWebView().value(forKey: "userAgent") as? String ?? ""
        if !userAgent.isEmpty {
            userAgent = userAgent + " otplesssdk"
        } else {
            userAgent = "iOS otplesssdk"
        }
        deviceInfo["userAgent"] = userAgent
            
        completion(Utils.convertDictionaryToString(deviceInfo))
